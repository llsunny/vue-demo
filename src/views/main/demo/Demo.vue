 ...
 * @author: llsunny
 * @lastEditor: llsunny
 * @update: 2022-11-03 14:13:29
 */
<!--
 * @Author: z_ly
 * @Description:
 * @FilePath: \vue-demo\src\views\main\demo\demo.vue
 *
-->
<template>
  <div class="demo">
    ...
    >
      <BaseInput
        ...
        @keyup.enter.native="e => handleEnter(e)"
      ></BaseInput>
    </div>
      ...
      <template v-for="item in tbTitle">
        ...
        >
          <template slot-scope="scope">
            <template v-if="scope.rowIndex === currentRowIndex">
              <el-input-number
                :ref="scope.rowIndex"
                ...
                :min="-9999999999"
                :max="9999999999"
                :precision="scope.row.resultPrecision"
                :controls="false"
                size="mini"
                @focus="handleFocus(scope.row, scope.rowIndex)"
                @blur="handleBlur(scope.row, scope.rowIndex)"
  ...
  methods: {
    ...
    handleEnter(e) {
      ...
      this.$nextTick(() => {
        ...
        this.$refs[`baseInput${this.curIndex}`][0].$children[0].focus()
      })
    },
    changeValue() {
      this.list[1].value = '333'
    },
    showOrHide() {
    ...
    // 手动切换选中行
    handleCurrentChange({row, rowIndex}) {
